...
// Quick access card to jump to This or That game
const ThisOrThatAccessCard = ({ onPress }: { onPress: () => void }) => (
  <TouchableOpacity 
    style={{
  ...
  >
    <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
      <MaterialCommunityIcons name="cards-playing" size={32} color={COLORS.darkNavy} />
      <View>
        <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 14, color: COLORS.darkNavy }}>
          Try This or That
        </Text>
        <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 12, color: COLORS.darkNavy, opacity: 0.7 }}>
          Get recipe ideas fast
        </Text>
      </View>
    </View>
    <MaterialCommunityIcons name="arrow-right" size={20} color={COLORS.darkNavy} />
  </TouchableOpacity>
);
    ...
    <SafeAreaView style={layoutStyles.safeArea}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView style={layoutStyles.container} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <ThisOrThatAccessCard onPress={() => router.push('/(tabs)/thisorthat')} />

// app/planWeek.tsx — "Plan for the Week"
//
// This is the question quiz that used to live inside This or That as its
// "weekly" mode. This or That is now a picture game (app/thisorthat.tsx),
// so the quiz got its own screen and its own button on Home. The
// "Cook from my pantry" mode that used to sit next to it moved to the
// Pantry tab (→ app/cookWithPantry.tsx).
//
// Flow: questions → vibe → your meal. Weekly means "shop for it" — we
// deliberately ignore what's on hand, so there's no pantry gating here.
...
import type { Vibe, ScoredRecipe } from '@/types';

type Screen = 'quiz' | 'vibe' | 'result';
...
export default function PlanWeekScreen() {
  ...
  const goBack = () => {
    if (qIndex === 0) {
      // No mode screen anymore — backing out of the first question exits.
      exitToHome();
      return;
    }
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        {qIndex === 0 && (
          <View style={styles.introWrap}>
            <Text style={styles.introTitle}>Plan for the week 📅</Text>
            <Text style={styles.introSub}>
              {TOTAL_QUESTIONS} quick questions, different every time — we'll find a
              meal worth shopping for. Skipping never counts against your results.
            </Text>
          </View>
        )}
  ...
  const opts = {
    ...
    vibe,
    preferences,
    // Weekly = shop for it. No pantry gating on purpose.
    requirePantryMatch: false,
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <Header
        ...
        center={<Text style={styles.headerTitle}>Your meal for the week</Text>}
        right={
          <TouchableOpacity
      ...
      <ScrollView contentContainerStyle={[styles.resultPad, scrollPad]} showsVerticalScrollIndicator={false}>
        ...
        {alternates.length > 0 && (
          <>
            <View style={styles.altHeader}>
              <Text style={styles.altHeading}>Other options for your week</Text>
        ...
        >
          ...
          <Text style={styles.primaryBtnText}>Plan another meal</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  introWrap: { paddingHorizontal: 20, marginBottom: 6 },
  introTitle: { fontSize: 24, fontWeight: '700', color: COLORS.darkNavy },
  introSub: { fontSize: 13, color: COLORS.textMuted, marginTop: 6, lineHeight: 19 },

  badgeRow: { paddingHorizontal: 20, marginBottom: 4, marginTop: 8 },
  catBadge: {
